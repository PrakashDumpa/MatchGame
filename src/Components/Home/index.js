...
class Home extends Component {
  ...
  componentDidMount() {
    this.setState({randImg: imagesList[0].imageUrl})
    clearInterval(this.timerID)
    this.timerID = setInterval(this.timeFunction, 1000)
  }
  ...
  clickingTabsButton = event => {
    this.setState({activeTabId: event.target.id})
  }

  tabsItems = (eachItem, isTrue) => {
    const {displayText, tabId} = eachItem
    const classNm = isTrue ? 'button_style' : 'text-light'
    return (
      <div>
        <button
          type="button"
          className={`h2 button ${classNm}`}
          id={tabId}
          onClick={this.clickingTabsButton}
        >
          {displayText}
        </button>
      </div>
  ...
  randomImages = () => {
    this.index = parseInt(Math.random() * imagesList.length)

    return {
      randImageUrl: imagesList[this.index].imageUrl,
      imageId: imagesList[this.index].id,
    }
  }

  onImageButtonClick = clickingId => {
    const {activeImageId} = this.state
    const {randImageUrl, imageId} = this.randomImages()
    if (activeImageId === clickingId) {
      this.setState(prevState => ({
        randImg: randImageUrl,
        activeImageId: imageId,
        count: prevState.count + 1,
      }))
    } else {
      this.setState(prevState => ({
        isNotMatch: !prevState.isNotMatch,
        time: 0,
        activeTabId: tabsList[0].tabId,
      }))
    }
  ...
  imagesFunction = eachImg => {
    ...
    return (
      <button
        ...
        onClick={() => this.onImageButtonClick(id)}
      >
        <img className="thumbnailImages" src={thumbnailUrl} alt="thumbnail" />
      </button>
    )
  }
  ...
  scoreCard = () => {
    ...
    return (
      <div className="score_division">
        <div className="scoreCard_container mb-5 p-5 text-light">
          ...
          <p className="h2">Your Score</p>
          <h1>{count}</h1>
          <button
            type="button"
            className="play_again_button p-2"
          ...
          >
            <img
              ...
              alt="reset"
            />
            <p className="m-0 text-light pl-2">PLAY AGAIN</p>
          </button>
        </div>
      </div>
    )
          ...
          {isNotMatch ? (
            ...
            <div className="main_Container">
              <div className="display_image_container">
                <img className="w-100 h-100" src={randImg} alt="match" />
              </div>
              <ul className="list-unstyled tabItems mt-3 mb-3">
                {tabsList.map(each => {
                  const isTrue = activeTabId === each.tabId
                  return (
                    <li key={each.tabId}>{this.tabsItems(each, isTrue)}</li>
                  )
                })}
              </ul>
              <ul className="list-unstyled imagesList">
                {filteredList.map(eachImg => (
                  <li key={eachImg.id} className="m-2">
                    {this.imagesFunction(eachImg)}
                  </li>
                ))}
              </ul>
            </div>
